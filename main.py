...
app.layout = dbc.Container([
    ...
    dbc.Row([
        dbc.Col(html.Label("Агрегация:", className='m-2'), width=12)
    ]),
    dbc.Col([
...
def update_graph1(n_clicks, contents, filename, selected_variable):
    if n_clicks > 0 and contents is not None and selected_variable is not None:
        save_uploaded_file(contents, filename)
        ds = xr.open_dataset(filename)
        var = ds[selected_variable]
        lon = ds['longitude'].values
        lat = ds['latitude'].values
        var_mean = var.mean(dim='time')  # You can change the aggregation method here
